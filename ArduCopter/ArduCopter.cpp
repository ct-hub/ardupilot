...
{
    // simulation
    bool openmv_new_data = false;
    if(control_mode != GUIDED) {
        openmv_new_data = false;

    } else {
        openmv.cx = 110;
        openmv.cy = 60;
        openmv_new_data = true;
    }
    // end of simulation code

    if(openmv.update() || openmv_new_data) {
        Log_Write_OpenMV();
        ...
        if( openmv.cx = 110) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,"Draw star finished, now go into DRAWSTAR Mode.");
            copter.set_mode(DRAWSTAR, MODE_REASON_MISSION_END);
        }
    }
}
